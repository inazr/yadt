...
- **Auto-parse on save** now runs `dbt parse` with the target selected in YADT, so `manifest.json` (and every `ref()` resolved from it, e.g. by dbt Charts) points at that target's relations. Switching the target re-parses right away
